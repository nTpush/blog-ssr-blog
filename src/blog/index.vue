    ...
    <div :class="$style['content']">
      ...
      <div :class="$style['body']" ref='body'>
        <p :class="$style['phoneSearh']">
         <input type="text" autofocus="autofocus" placeholder="寻 找 真 正 的 好 代 码" @change="handleSearch" @enter="handleSearch" v-model="keyword">
        </p>
        ...
        <div :class="$style['search']">
          <p>什么才是真正的好代码？</p>
          <input type="text" autofocus="autofocus" placeholder="寻 找 真 正 的 好 代 码" @change="handleSearch" @enter="handleSearch" v-model="keyword">
          <h6 v-if="note"><span>温馨提示: {{note}}</span></h6>
        </div>
            ...
            >
              <div>
                <div :class="$style['item']">
                    <div :class="$style['type1']" v-if="!(item.type !== '' && Number(item.type) === 0)">
                        <div>
                          <p :class="$style['title']">{{ item.title }}</p>
                          <div :class="$style['tag']" >
                            <div>
                              <span :style="{background: `#${tagColor[index]}`}" v-for="(tag, index) in item.mate_id" :key="index">{{ tag.name }}</span>
                            </div>
      ...
      <div v-if="loading" :class="$style['loading']"><img src="../assets/images/Preloader_7.gif" alt=""></div>
      <!-- <p ><img src='../assets/images/Preloader_7.gif' v-if="loading"/></p> -->
    </div>
  </div>
</template>
<script>
import { LODA_ARTICLES_ASYNC, ADD_ARTICLE_ASYNC } from '@/components/Article/module'
...
export default {
  ...
  methods: {
    ...
    MouseWhellDefault() {
      ...
      if (typeof window === 'object') {
        let scrollTop = window.innerWidth > 768 ? this.$refs['wrapper'].scrollTop : document.documentElement.scrollTop
        let heightU = window.innerWidth > 768 ? this.$refs['wrapper'].scrollHeight : document.documentElement.scrollHeight
        let heightW = window.innerWidth > 768 ? this.$refs['wrapper'].clientHeight : document.documentElement.clientHeight
    ...
    getPuBuLiuShow(first = 0) {
      ...
      setTimeout(() => {
        ...
        if (window.innerWidth > 768) {
          ...
          this.firefox ? wrapper.removeEventListener('DOMMouseScroll', this.MouseWhellDefault, false) : (wrapper.onscroll = this.MouseWhellDefault)
        } else {
          this.firefox ? wrapper.removeEventListener('DOMMouseScroll', null, false) : (wrapper.onscroll = null)
